...
class ChooseMap:
    def __init__(self):
        ...
        self.scale = 0.5 * ratio
        self.back_scale = 2 * ratio
        # Load hình ảnh của các map và điều chỉnh kích thước lên 30%
        self.maps = [pygame.transform.scale(pygame.image.load(f'./assets/background/map{i}40.png'), (int(self.scale * pygame.image.load(f'./assets/background/map{i}40.png').get_width()), int(self.scale * pygame.image.load(f'./assets/background/map{i}40.png').get_height()))) for i in range(1, 6)]
        # Load hình ảnh cho nút "back" scale 30%
        self.back_button_img = pygame.transform.scale(pygame.image.load('assets/background/ENG/back.png'), (0.3 * int(self.back_scale * pygame.image.load('assets/background/ENG/back.png').get_width()), 0.3 * int(self.back_scale * pygame.image.load('./assets/background/ENG/back.png').get_height())))
        # Đặt nút "Back" ở giữa màn hình
        self.back_button = pygame.draw.rect(self.screen, (255, 165, 0), (self.window_width / 2 - self.back_button_img.get_width() / 2, self.window_height - self.window_height / 9, self.back_button_img.get_width() * self.bakc, self.back_button_img.get_height()))
    def draw(self):
        ...
        for i, map_img in enumerate(self.maps):
            if i < 3:
                ...
                x = self.window_width / 2 - int(self.scale * pygame.image.load(f'./assets/background/map{i + 1}40.png').get_width()) / 2 + (i - 1)*(int(self.scale * pygame.image.load(f'./assets/background/map{i + 1}40.png').get_width()) / 5 * 3 + int(self.scale * pygame.image.load(f'./assets/background/map{i + 1}.png').get_width()) / 10 * 3)
            else:
                y = self.window_height / 9 * 5  # Điều chỉnh vị trí y của hàng thứ hai
                x = self.window_width / 2 - (int(self.scale * pygame.image.load(f'./assets/background/map{i + 1}40.png').get_width()) / 3 * 4) + (i - 3)*(5*int(self.scale * pygame.image.load(f'./assets/background/map{i + 1}40.png').get_width()) / 3)
